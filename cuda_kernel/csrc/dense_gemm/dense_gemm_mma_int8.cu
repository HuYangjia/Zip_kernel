...
//
// Uses mma.m16n8k32.s8.s8.s32 for the inner product.  Weights are
// decoded from packed s4 to s8 at shmem-stage time; both operands
// are consumed via scalar shmem reads that match the mma fragment
// layout per-lane exactly.
//
// Single-buffered: total static shared memory = 32 KB for kBn=128,
// fitting within SM89's 48 KB static-shmem budget without spill to
// dynamic shmem.  HBM/MMA overlap can later be reintroduced via
// cp.async + register double-buffering without changing shmem.
...
__global__ void dense_gemm_mma_int8_kernel(
    ...
    for (int g = 0; g < n_groups; ++g) {
        ...
        for (int ks = 0; ks < kKSteps; ++ks) {
            ...
            // A operand: scalar shmem reads matching m16n8k32.s8 A layout.
            ...
            // B operand: scalar shmem reads matching m16n8k32.s8 B layout.
        ...
        __syncthreads();  // before next iteration overwrites sW/sX
    }
...
void launch(
    ...
    TORCH_CHECK(X_s4.size(1) == d_in_half);
