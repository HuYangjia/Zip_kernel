...
# ---------------------------------------------------------------------------
# Canary: SPLIT_K=1 bit-exact vs dense_gemm_u4_s4_to_out
# ---------------------------------------------------------------------------
...
def test_splitk_equals_1_bit_exact(T, d_out, d_in):
    """SPLIT_K=1 must be bit-exact vs dense_gemm_u4_s4_to_out."""
    ...
    if not torch.equal(Y_ref, Y_new):
        ...
        pytest.fail(
            f"SPLIT_K=1 not bit-exact at (T={T}, d_out={d_out}, d_in={d_in}); "
...
def test_auto_split_k_close_to_reference(T, d_out, d_in):
    ...
    if chosen == 1:
        # SPLIT_K=1 auto choice must still be bit-exact.
        assert torch.equal(Y_ref, Y_new), (
            f"auto SPLIT_K=1 not bit-exact at (T={T}, d_out={d_out}, d_in={d_in})"
        )
    else:
        torch.testing.assert_close(
            Y_new.float(), Y_ref.float(),
